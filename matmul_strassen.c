/*
 * gcc -O2 -fopenmp -std=c99 -o matmul_strassen matmul_strassen.c -lm
 * uso: ./matmul_strassen [N]   (N debe ser potencia de 2, defecto 512)
 */
...
{
    if (n <= BS) { mseq_naive(A, B, C, n); return; }
    int h = n/2;
...
static void str_par(const double *A, const double *B, double *C, int n, int d)
{
    strassen(A, B, C, n, d);
}
