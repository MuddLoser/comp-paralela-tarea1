/*
 * gcc -O2 -fopenmp -std=c99 -o matmul_hibrido matmul_hibrido.c -lm
 * uso: ./matmul_hibrido [N]   (N debe ser potencia de 2, defecto 512)
 */
...
/* Caso base: multiplicacion por bloques (tiling) cache-friendly */
...
/* Strassen recursivo con caso base por bloques (hibrido) */
static void strassen(const double *A, const double *B, double *C, int n, int d)
{
    if (n <= BS) { mseq(A, B, C, n); return; }
    int h = n/2;
    ...
    /* Los 7 productos son independientes: se pueden ejecutar en paralelo */
    #pragma omp parallel sections if(d < 3) \
        shared(m1,m2,m3,m4,m5,m6,m7,a11,a12,a21,a22,b11,b12,b21,b22) \
        firstprivate(h,d) default(none)
    {
    #pragma omp section
    { double *u=mnew(h),*v=mnew(h); madd(u,a11,a22,h); madd(v,b11,b22,h);
      strassen(u,v,m1,h,d+1); free(u); free(v); }

    #pragma omp section
    { double *u=mnew(h); madd(u,a21,a22,h);
      strassen(u,b11,m2,h,d+1); free(u); }

    #pragma omp section
    { double *u=mnew(h); msub(u,b12,b22,h);
      strassen(a11,u,m3,h,d+1); free(u); }

    #pragma omp section
    { double *u=mnew(h); msub(u,b21,b11,h);
      strassen(a22,u,m4,h,d+1); free(u); }

    #pragma omp section
    { double *u=mnew(h); madd(u,a11,a12,h);
      strassen(u,b22,m5,h,d+1); free(u); }

    #pragma omp section
    { double *u=mnew(h),*v=mnew(h); msub(u,a21,a11,h); madd(v,b11,b12,h);
      strassen(u,v,m6,h,d+1); free(u); free(v); }

    #pragma omp section
    { double *u=mnew(h),*v=mnew(h); msub(u,a12,a22,h); madd(v,b21,b22,h);
      strassen(u,v,m7,h,d+1); free(u); free(v); }
    }

    /* Combinacion: C11=M1+M4-M5+M7, C12=M3+M5, C21=M2+M4, C22=M1-M2+M3+M6 */
...
static void hyb_par(const double *A, const double *B, double *C, int n, int d)
{
    strassen(A, B, C, n, d);
}
